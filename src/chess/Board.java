...
public class Board 
{
    ...
    public void setPiece(Position position, Piece piece,String promote){
        Position oldPosition = piece.getPosition();
        if((piece instanceof Pawn) && ((Pawn)piece).canPromote(position)){
            switch(promote){
                case "N": 
                    board[position.getRank()-1][position.getFile()] = new Knight(piece.getColor(),position);
                    break;
                case "R":
                    board[position.getRank()-1][position.getFile()] = new Rook(piece.getColor(),position);
                    break;
                case "B":
                    board[position.getRank()-1][position.getFile()] = new Bishop(piece.getColor(),position);
                    break;
                default:
                    board[position.getRank()-1][position.getFile()] = new Queen(piece.getColor(),position);
                    break;
            }
        }
        else{
